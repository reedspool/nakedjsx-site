...
<script>
  {
    ...
    window.startCountdown8 = () => {
      ...
      setTimeout(() => {
        const { total, count, average } = averageLoopTime(5);
...
<script>
  {
    ...
    window.startCountdown9 = async () => {
      output.innerHTML = "Running..."
      const { total, count, average } = await averageSetTimeoutTime(5);
      output.innerHTML = average + " average milliseconds per loop after "  + count + " loops."
    }
  }
</script>
...
A number over 4 milliseconds here was evidence that the [HTML spec was being followed](https://html.spec.whatwg.org/multipage/timers-and-user-prompts.html#timers).
