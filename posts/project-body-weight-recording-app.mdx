...
<Future>[This pen by Carter Lovelace](https://codepen.io/carterfromsl/pen/QWYMjBW) inspired me to try a neo-brutalist style.</Future>
...
<Future>While searching for how to do this, I found [this additional doc on Supabase SSR](https://supabase.com/docs/guides/auth/server-side/creating-a-client) which described an additional package and a different method for creating a client on the server. I decided not to try this at that moment, but made a note to try it later.</Future>
...
So that was my login page working. Next I wanted to go to a page which showed all my entries in a table. To do that, I made an endpoint on my server which served that table, populated via server-side authentication.
...
<Future>I made a table to show data</Future>

<Future>I made a form to upload new data.</Future>
