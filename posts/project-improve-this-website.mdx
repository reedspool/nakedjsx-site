...
<Future>I had a specific issue prototyping with the NakedJSX's client side scripting method. Maybe it was all JavaScript models? Yes, I think it was. The issue as I saw it was that I wanted to write all of my code in public. When I opened the dev tools on my web page, I wanted the maximum access to everything I was working on. I might want to try out some code or otherwise. The rule of encapsulation makes sense for code which is meant to be shared, but when I am making an application, that's not shared code! I should hope all the code which I bring in from external sources, ror which I may want to share with othrs my
lf has thee properties of encapsulation and good citizenship (not polluting). But I want my tools to be available to me easily. To be very specific, I do not like havingto explcitly attach a functiono the window object for it to be global. I want global to be the default on all the code I'm writing.</Future>
