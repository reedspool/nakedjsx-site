...
It was pretty fast too. I made 1000 copies of a 2.5kb post file and it took 18.5 seconds to run the compilation command.
...
I was not a fan of CSS-in-JS when I used it, but I was a fan of Tailwind. To implement it, I knew I 'd need these steps:
...
Every time I edited a post, all of the posts were subsequently recompiled. Ideally, only the post I had edited and saved would be recompiled.

When I edited a file outside my posts directory, the development server would regenerate only that file which was took less than half a second.

The documentation also promised that the page would refresh which I didn't experience on either posts or other JSX files.

So far I hadn't had to dig in and understand the source code of NakedJSX to solve any of my issues because the developer was so generous with his time. I decided to push all my current work to GitHub and point him to it to ask for his help.
...
So I tried it. I used Node's `fs.access` and `fs.stat` to read the input and the output and determine which was newer. I found the same issue which the NakedJSX developer mentioned. Unfortunately, the development server wouldn't register any files which were not exported, so those with no changes would never be recompiled again. To get around this for now, I ended up writing imports for each file but including a flag as to whether it should be recompiled or not.

Unfortunately this still did not solve the issue. It seemed to my eyes that when one of my post files changed, `all.mjs` was not re-run. Was its output being cached? I never saw my `console.log`s from `all.mjs` again after the first build.

A small annoyance I had was that the development server did not respond to Control-C. Instead, the development server responded to "x" to exit. I was so used to Control-C to exit out of other programs, I was repeatedly confused when nothing happened.

Another random issue I found is that the information I logged with `console.log` was overwritten by the output of the server. I wasn't sure what was going on there.
