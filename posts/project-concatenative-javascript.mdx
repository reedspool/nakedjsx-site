...
export const tests = [
  ...
  { 
    Title: () => <>Click event</>,
    code: "[ ' ✅' event target> >text ] ' click' me .addEventListener"
  },
]

export const Test = ({ Title, code }) =>
  <>
    <tr> 
      <td><Title /></td> 
      <td> <span c={code}>🚧</span> </td> 
      <td><button class="cpnt-button" _="on click toggle .hidden on next <tr /> then if my innerText is 'Show' put 'Hide' into me else put 'Show' into me">Show</button></td>
    </tr>
    <tr class="hidden">
...
<table>
    ...
    <tbody>
    {tests.map(({ Title, code }) => <Test Title={Title} code={code} />)}
    </tbody>
</table>
