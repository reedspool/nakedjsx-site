...
export const tests = [
  ...
  { 
    Title: () => <><code>on click</code></>,
    code: "on click ' ✅' me >text ;",
    TestTarget : () => <><span _="init wait a tick then send click to closest <[c] />">🚧</span></> 
  },
]
...
export const Test = ({ Title, code, TestTarget = () => <>🚧</> }) =>
  <>
    <tr> 
      ...
      <td><button class="cpnt-button" c="on click me text> ' Show' === if ' Hide' else ' Show' endif me >text " _="on click toggle .hidden on next <tr />">Show</button></td>
    </tr>
    <tr class="hidden">
      <td class="px-md py-sm bg-gray-300 text-gray-700" colspan={2}><code class="text-xs">{code}</code></td>
    </tr>
  </>
