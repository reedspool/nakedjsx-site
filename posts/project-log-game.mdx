...
The source code of the game lives in the NakedJSX client JavaScript file for my site's posts, [here (local, won't work on the web)](../src/compile-all-posts-client.jsx) or <GitHubLink extraPath="/blob/main/src/compile-all-posts-client.jsx#L1">here (global, GitHub)</GitHubLink>.
...
I decided to lean in to the "log" aspect of the game with some gameplay elements relying on interacting with and interpreting the log. For example, I decided to add lots of debugging information to the actual game log and then filter it out. I thought the player could use the same filtering for important messages. For example, if widgets only ever appeared in the scrolling chat log, then the widgets would roll off the top as new chat rolled in and force the player to scroll back to interact. And then if a new widget came in, then they'd be scrolling back and forth to interact with both. A solution to that would be a filter for "only widgets". 

To further lean in, I thought that instead of clearing/deleting the past log, they could simply be disabled and marked as an archive. So the player could still scroll back to read the words. And also they'd have the problem of avoiding the archives when scrolling back.
