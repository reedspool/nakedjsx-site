...
I realized this auth stuff was all open-ended. Although there were strong patterns in the documentation, the exact steps I took and decisions I made about auth would rely on decisions of the concrete application I wanted to make. That thought was driven by Supabase's focus on "row level policies". So my auth policies would match exactly to the shape of my database.

To that end, I made a decision of a first goal. I weighed myself just before I began working this morning. I decided to make a table to record my daily body weight records. So I switched gears to create a table for those entries.

Then I added Row Level checks to only allow users to read their own records and write records with their own user ID.
